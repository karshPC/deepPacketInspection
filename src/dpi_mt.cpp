...
public:
    ...
    DPIEngine(const Config& cfg) : config_(cfg) {
        ...
        std::cout << "\n";
        std::cout << border("╔", "═", "╗") << "\n";
        printLine(centered("DPI ENGINE v2.0 (Multi-threaded)"));
        std::cout << border("╠", "═", "╣") << "\n";
        {
            std::ostringstream l;
            l << "Load Balancers: " << cfg.num_lbs
...
private:
    ...
    static constexpr int BOX_WIDTH = 66;

    std::string border(const std::string& left, const std::string& fill, const std::string& right) {
        std::string s = left;
        for (int i = 0; i < BOX_WIDTH + 2; i++) s += fill;
        s += right;
        return s;
    }
    ...
    void printLine(const std::string& content) {
        ...
        if ((int)c.size() > BOX_WIDTH) c = c.substr(0, BOX_WIDTH);
        std::cout << "║ " << std::left << std::setw(BOX_WIDTH) << c << " ║\n";
    }

    std::string centered(const std::string& text) {
        int pad = BOX_WIDTH - (int)text.size();
    ...
    void printReport() {
        std::cout << "\n";
        std::cout << border("╔", "═", "╗") << "\n";
        printLine(centered("PROCESSING REPORT"));
        std::cout << border("╠", "═", "╣") << "\n";

        { std::ostringstream l; l << "Total Packets:  " << stats_.total_packets.load(); printLine(l.str()); }
        { std::ostringstream l; l << "Total Bytes:    " << stats_.total_bytes.load(); printLine(l.str()); }
        { std::ostringstream l; l << "TCP Packets:    " << stats_.tcp_packets.load(); printLine(l.str()); }
        { std::ostringstream l; l << "UDP Packets:    " << stats_.udp_packets.load(); printLine(l.str()); }

        std::cout << border("╠", "═", "╣") << "\n";
        { std::ostringstream l; l << "Forwarded:      " << stats_.forwarded.load(); printLine(l.str()); }
        { std::ostringstream l; l << "Dropped:        " << stats_.dropped.load(); printLine(l.str()); }

        std::cout << border("╠", "═", "╣") << "\n";
        printLine("THREAD STATISTICS");
        for (size_t i = 0; i < lbs_.size(); i++) {
            std::ostringstream l;
            l << "  LB" << i << " dispatched: " << lbs_[i]->dispatched();
            printLine(l.str());
        }
        for (size_t i = 0; i < fps_.size(); i++) {
            std::ostringstream l;
            l << "  FP" << i << " processed:  " << fps_[i]->processed();
            printLine(l.str());
        }

        std::cout << border("╠", "═", "╣") << "\n";
        printLine(centered("APPLICATION BREAKDOWN"));
        std::cout << border("╠", "═", "╣") << "\n";
        ...
        for (const auto& [app, count] : sorted_apps) {
            ...
            std::string label = appTypeToString(app);
            if (rules_.isAppBlocked(app)) label += " (BLOCKED)";
            std::ostringstream l;
            l << std::setw(20) << std::left << label
